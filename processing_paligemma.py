...
def add_image_tokens_to_prompt(prefix_prompt,bos_token,image_seq_len,image_token):
    ...
    ## This function forms the pattern of the tokens to feed the decoder  
    ## [<image><image><image>.....<BOS><text><text>]
...
def process_images(
    ...
    #Rescales the pixel values to be in the range of [0,1]
    images=[rescale(image,scale=rescale_factor) for image in images]

    #Normalise the images to have mean as 0 and the std as 1
...
class PaliGemmaProcessor:
    ...
    def __call__(
        ...
        assert len(images)==1 and len(text)==1, f"Received {len(images)} images for the {len(text)} prompts."
        ...
        # Concatenating all the images into a single array of dimension [Batch_size,num_channles,height,width]
        pixel_values=np.stack(pixel_values,axis=0)

        #Converting the final array to tensor
